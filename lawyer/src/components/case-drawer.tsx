...
import { addNote, addPayment, deleteFile, deleteNote, deletePayment, getCaseDetails, regenerateQr, updateCase, updateQrSettings, uploadFiles } from '@/lib/api';
...
export function CaseDrawer({ item, open, onClose, onRefresh }: { item: CaseItem | null; open: boolean; onClose: () => void; onRefresh: () => void; }) {
  ...
  return <Modal open={open} onClose={onClose} title={current.title} wide>
    ...
    {tab==='overview' && <div className="grid two">
      ...
      <div className="actions"><button className="primary-btn" onClick={async()=>{ await updateCase(current.id, current); await load(); onRefresh(); }}>{t('save')}</button></div>
    </div>}
    ...
    {tab==='files' && <div>
      ...
      <div className="file-grid">{files.map(f => <div key={f.id} className="file-card"><a href={f.file_url || '#'} target="_blank">{f.file_name}</a><small>{f.mime_type}</small><button className="ghost-btn" onClick={async()=>{ await deleteFile(f); await load(); onRefresh(); }}>حذف</button></div>)}</div>
    </div>}

...
export function CaseDrawer({ item, open, onClose, onRefresh }: { item: CaseItem | null; open: boolean; onClose: () => void; onRefresh: () => void; }) {
  ...
  return <Modal open={open} onClose={onClose} title={current.title} wide>
    ...
    {tab==='payments' && <div>
      ...
      <div className="actions"><button className="primary-btn" onClick={async()=>{ await addPayment(current.id, paymentForm.amount, paymentForm.payment_date, paymentForm.note); setPaymentForm({ amount: 0, payment_date: today(), note: ''}); await load(current.id); onRefresh(); }}>{t('addPayment')}</button></div>
      <table className="table"><thead><tr><th>{t('amount')}</th><th>{t('date')}</th><th>{t('note')}</th><th></th></tr></thead><tbody>{payments.map(p=><tr key={p.id}><td>{currency(p.amount)}</td><td>{p.payment_date}</td><td>{p.note || '-'}</td><td><button className="ghost-btn" onClick={async()=>{ await deletePayment(p.id, current.id); await load(current.id); onRefresh(); }}>حذف</button></td></tr>)}</tbody></table>
    </div>}

    {tab==='files' && <div>
      <div className="actions"><label className="primary-btn file-btn">{t('uploadFiles')}<input type="file" multiple hidden onChange={async(e)=>{ if (e.target.files?.length) { await uploadFiles(current.id, Array.from(e.target.files)); await load(current.id); onRefresh(); }}} /></label><label className="ghost-btn file-btn">{t('cameraUpload')}<input type="file" accept="image/*" capture="environment" multiple hidden onChange={async(e)=>{ if (e.target.files?.length) { await uploadFiles(current.id, Array.from(e.target.files)); await load(current.id); onRefresh(); }}} /></label></div>
      <div className="file-grid">{files.map(f => <div key={f.id} className="file-card"><a href={f.file_url || '#'} target="_blank">{f.file_name}</a><small>{f.mime_type}</small><button className="ghost-btn" onClick={async()=>{ await deleteFile(f.id, current.id); await load(current.id); onRefresh(); }}>حذف</button></div>)}</div>
    </div>}

    {tab==='notes' && <div><div className="actions"><input value={noteText} onChange={(e)=>setNoteText(e.target.value)} placeholder={t('note')} /><button className="primary-btn" onClick={async()=>{ if (!noteText) return; await addNote(current.id, noteText); setNoteText(''); await load(current.id); }}> {t('addNote')}</button></div><div className="stack">{notes.map(n=><div key={n.id} className="note-card"><p>{n.content}</p><small>{new Date(n.created_at).toLocaleString()}</small><button className="ghost-btn" onClick={async()=>{ await deleteNote(n.id, current.id); await load(current.id); }}>حذف</button></div>)}</div></div>}
    ...
    {tab==='qr' && <div className="qr-wrap">{qrDataUrl && <img src={qrDataUrl} alt="qr" className="qr-image" />}<div className="stack"><button className="primary-btn" onClick={()=>copyText(`${window.location.origin}/share/${current.qr?.token}`)}>{t('copyLink')}</button><button className="ghost-btn" onClick={()=>window.open(`/share/${current.qr?.token}`, '_blank')}>{t('openPublicPage')}</button><button className="ghost-btn" onClick={async()=>{ await regenerateQrToken(current.id); await load(current.id); onRefresh(); }}>{t('regenerateQr')}</button><label className="switch-row"><input type="checkbox" checked={!!current.qr?.allow_download} onChange={async(e)=>{ await updateQrSettings(current.id, { allow_download: e.target.checked }); await load(current.id); }} /> {t('allowDownload')}</label><label className="switch-row"><input type="checkbox" checked={!!current.qr?.show_client_name} onChange={async(e)=>{ await updateQrSettings(current.id, { show_client_name: e.target.checked }); await load(current.id); }} /> {t('showClientName')}</label><label className="switch-row"><input type="checkbox" checked={!!current.qr?.is_active} onChange={async(e)=>{ await updateQrSettings(current.id, { is_active: e.target.checked }); await load(current.id); }} /> Active</label></div></div>}
  </Modal>;
}

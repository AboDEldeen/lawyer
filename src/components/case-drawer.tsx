...
export function CaseDrawer({ item, open, onClose, onRefresh }: CaseDrawerProps) {
  ...
  const handleDelete = async () => {
    if (!currentCase) return;
    if (window.confirm(t('confirmDelete') || 'هل أنت متأكد من حذف هذه القضية؟')) {
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <button className="ghost-btn danger" onClick={async () => {
                        if (confirm(t('confirmDeletePayment') || 'حذف هذه الدفعة؟')) {
            ...
            <div className="file-grid">
              {files.map(f => (
                <div key={f.id} className="file-card">
                  ...
                  <button className="ghost-btn danger" onClick={async () => {
                    if (confirm(t('confirmDeleteFile') || 'حذف هذا الملف؟')) {
            ...
            <div className="stack">
              {notes.map(n => (
                <div key={n.id} className="note-card">
                  ...
                  <button className="ghost-btn danger" onClick={async () => {
                    if (confirm(t('confirmDeleteNote') || 'حذف هذه الملاحظة؟')) {
